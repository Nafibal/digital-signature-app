...
Description:
The system must persist document progress, content, PDFs, and signing state.

Acceptance Criteria:

- Given partial progress, when the user exits, then progress is saved including current step and sub-step.
- Given a saved document, when the user returns, then the workflow resumes from the last step.
- Given multiple documents, when listing documents, then each document is distinguishable by title, status, and creation date.
- Given a document with content, when the user returns, then the HTML content is restored in the editor.
- Given a document with uploaded PDF, when the user returns, then the PDF is loaded from Supabase Storage.
...
- Security:
  ...
  - Auth mechanism: Better Auth with secure session management
  - Password hashing: Secure hashing via Better Auth provider
