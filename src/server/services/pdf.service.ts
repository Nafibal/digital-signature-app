 ...
 * PDF Service
 *
 * PDF generation and processing logic
 */

import { chromium } from "playwright";
import { prisma } from "../db";
import { uploadPDF } from "../storage/upload";
...
export async function generatePdfFromHtml(
  ...
  try {
    ...
    const pdfFile = new File([pdfUint8Array], `document-${documentId}.pdf`, {
      type: "application/pdf",
    });

    // Upload PDF to Supabase Storage
    const uploadResult = await uploadPDF(pdfFile, userId);

    // Create DocumentPdf record
    const documentPdf = await createDocumentPdf({
      documentId,
